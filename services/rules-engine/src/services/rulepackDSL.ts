...
export class RulepackInterpreter {
  ...
  private evaluateCondition(condition: Condition, data: Record<string, unknown>): boolean {
    switch (condition.type) {
      ...
      case 'in_range':
        if (!condition.field || !condition.value || typeof condition.value !== 'object') return false;
  ...
  private compare(
    ...
    switch (operator) {
      case 'gt': return fieldNum > expectedNum;
      case 'gte': return fieldNum >= expectedNum;
      case 'lt': return fieldNum < expectedNum;
      case 'lte': return fieldNum <= expectedNum;
    ...
    try {
      ...
      if (calc.rounding) {
        ...
        switch (calc.rounding.method) {
          case 'round': return Math.round(numResult * factor) / factor;
          case 'floor': return Math.floor(numResult * factor) / factor;
          case 'ceiling': return Math.ceil(numResult * factor) / factor;
        }
      }
...
export class RulepackManager {
  ...
  async getActiveRulepack(
    ...
    return result.rows[0].compiled_json as Record<string, unknown>;
  }
