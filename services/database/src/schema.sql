...
CREATE TABLE validation_rejection_queue (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    run_id UUID NOT NULL REFERENCES validation_runs(id) ON DELETE CASCADE,
    domain VARCHAR(50) NOT NULL,
...
CREATE TABLE validation_audit_events (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    run_id UUID NOT NULL REFERENCES validation_runs(id) ON DELETE CASCADE,
    actor UUID,
    action VARCHAR(100) NOT NULL,
...
CREATE INDEX idx_validation_decisions_run ON validation_decisions(run_id);
CREATE INDEX idx_validation_rejection_queue_run ON validation_rejection_queue(run_id);
CREATE INDEX idx_validation_overrides_decision ON validation_overrides(decision_id);
CREATE INDEX idx_validation_audit_events_run ON validation_audit_events(run_id);
